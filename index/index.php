...
switch ($_SERVER['SERVER_ADDR']) {
    ...
    case '10.100.102.37':
        break; # www7
    case '161.246.34.33':
        break; # www1 public ip
    case '161.246.34.45':
        break; # www2 public ip
    case '161.246.34.46':
        break; # www3 public ip
    case '161.246.34.47':
        break; # www4 public ip
    default:
        /* require_once('../user/core.php');
        header("Content-type: text/html; charset=UTF-8");
        $restored_date = '';
        $rcon = replicateConnection();
        $sql = "SELECT `VALUE` FROM `master_reg_utility`.`DATABASE_METADATA` WHERE `KEY` = 'RESTORED_BACKUP_FILENAME'";
        $result = mysqli_query($rcon, $sql);
        if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $restored_date = str_replace('db_backup_', '', $row['VALUE']);
            $restored_date = str_replace('.tar.gz', '', $restored_date);
            $restored_date = '<br>' . $restored_date;
        } */
        echo '<p style="pointer-events: none; opacity: 0.8; position: fixed; bottom: 0; left:0; z-index: 999; color:red; background-color: white; font-size:35px;">' .
            '<b>เว็บไซต์นี้จัดทำเพื่อใช้ในการทดสอบเท่านั้น <br> This website is constructed for testing purposed only</b>' .
           /*  $restored_date . */
            '</p>';
        /* mysqli_close($rcon); */
}
...
<body>
    <div id="app">
        <div class="container-xl reg-page">
            ...
            <!-- ประกาศ -->
            <!--div class="ml-5 mr-5 mb-2 mt-2">
                        <a href="http://www.reg.kmitl.ac.th/uploads/PNG_2020-11-10_09-09-01_oT2yDz.png" target="blank" class="btn btn-block btn-lightgrey text-center">                            
                            <span class="h4 pl-2" style="line-height: 45px;">{{ t('Office of the Registrar will be closed on Wednesday, November 11, 2020.', 'สำนักทะเบียนและประมวลผล งดให้บริการ ในวันพุธที่ 11 พฤศจิกายน 2563') }}</span>
                        </a>
            </div>
			<!-- ---------------------------------------------------------- -->

            <!-- ประกาศ -->
            <div class="row reg-banner-margin shadow-sm bg-white pb-3" v-show="announceNow.length!==0">
                <div class="col-12 p-0">
                    <div class="col-12 pt-4 pl-5 mb-3">
                        <span class="h2 font-weight-bold linear-wipes text-dark">
                            <i class="icofont-alarm h1 text-primary"></i>
                            {{ t('Events', 'เหตุการณ์สำคัญ') }}
                        </span>
                    </div>
                    <div class="ml-5 mr-5 mb-3">
                        <a v-for="item in announceNow" v-bind:href="item.url" target="blank" class="btn btn-block btn-lightgrey text-left">
                            <i class="icofont-circled-right text-primary h5"></i>
                            <span class="h4 pl-2" style="line-height: 45px;">{{ t(item.textEN, item.textTH) }}</span>
                        </a>
                    </div>
                    <div class="m-2 mr-4 mt-0 text-right">
                        <a href="news.php?group=1">
                            <div class="btn btn-dark btn-sm ml-2">{{ t('See all', ' ดูประกาศทั้งหมด') }}</div>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <!-- ---------------------------------------------------------- -->
            <!-- ประกาศ -->
            <div class="row reg-banner-margin pt-3">
                <div class="col-12 col-md-6 p-0">
                    <div class="card m-3">
                        <div class="card-header text-dark bg-white font-weight-bolder bg-line-1 h4">
                            <span class="text-primary">{{ t('News', ' ประกาศ') }}</span>
                            {{ t('(Undergraduate)', ' ระดับปริญญาตรี') }}
                        </div>
                        <div class="card-body reg-card-scroll reg-card-height">
                            <div v-for="item in newsBachelor" class="btn-tran">
                                <a v-bind:href="item.url" class="btn btn-tran text-left mb-2" target="blank">
                                    <i class="icofont-caret-right text-primary" v-show="!item.pin"></i>
                                    <i class="icofont-tack-pin text-primary h3" v-show="item.pin"></i>
                                    <span>{{ t(item.textEN, item.textTH) }}</span>
                                </a>
                                <hr class="m-0">
                            </div>
                        </div>
                        <hr class="mb-0">
                        <div class="m-2 mr-4 mt-0 text-right">
                            <!-- <form action="search_news.php" target="_blank" method="post" class="form-inline text-right" enctype="multipart/form-data">
                                <div class="input-group input-group-sm">
                                    <input type="text" name="keyword_search" id="keyword_search1" class="form-control">
                ...
                <div class="col-12 col-md-6 p-0">
                    <div class="card m-3">
                        <div class="card-header text-dark bg-white font-weight-bolder bg-line-2 h4">
                            <span class="text-primary">{{ t(' News', ' ประกาศ') }}</span>
                            {{ t('(Graduate)', ' ระดับบัณฑิตศึกษา') }}
                        </div>
                        <div class="card-body reg-card-scroll reg-card-height">
                            <div v-for="item in newsGrad" class="btn-tran">
                                <a v-bind:href="item.url" class="btn btn-tran text-left mb-2" target="blank">
                                    <i class="icofont-caret-right text-primary" v-show="!item.pin"></i>
                                    <i class="icofont-tack-pin text-primary h3" v-show="item.pin"></i>
                                    <span>{{ t(item.textEN, item.textTH) }}</span>
